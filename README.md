...
- 在「设置 → 通用 → 多设备同步」中开启，三种模式：**关闭（默认）/ 托管（本机开启同步服务）/ 连接（连接其他设备的同步服务）**。关闭模式下零网络行为、零端口监听。
- **隐私边界**：同步内容仅限**日聚合**（日期 / 来源 / 模型 / 四列 token 计数）与设备名、应用版本、最后活跃时间——**绝不传输**转录内容、本地路径、凭据或账号身份。
- **⚠️ 网络暴露面披露**：托管模式会让应用在**所有网卡接口（`0.0.0.0:45539`）开启明文 HTTP 监听**（无 TLS），同一局域网内**持有访问密钥**的设备均可连接并读取全部设备的聚合数据；请只在可信的私人局域网内开启，勿在公共网络使用。
- **访问密钥**：128 位随机值（OS CSPRNG），仅生成时一次性显示，可随时重置（旧密钥立即失效）；所有请求经 `X-Pulse-Secret` 头鉴权，常量时间比较防时序枚举，失败一律 401。密钥只存 Windows 凭据管理器，不落设置文件。
- 趋势仪表盘顶部会随之出现「设备」行：每设备今日 tokens 与在线状态，支持「全部设备 / 本机 / 单设备」合并视图筛选。
...
4. **多设备同步的边界（opt-in）**：同步是默认关闭的**本机自有设备间直连**功能。开启「托管」后，本机会在局域网监听 45539 端口（明文 HTTP），仅与你**自己持有访问密钥的设备**共享日聚合与设备元信息（详见「多设备同步」一节）；这属于你自己的设备间通信，应用自身仍不连接任何开发者或第三方服务器，上列「零数据回传」口径不变。
