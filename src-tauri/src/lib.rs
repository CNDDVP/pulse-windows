    ...
    pub async fn refresh_account_now(app:&AppHandle,account_id:&str)->Result<u64,String>{
        ...
        if created{
            ...
            tauri::async_runtime::spawn(async move{
                ...
                if let Some(r)=reading {reading=Some(crate::stepfun_session::recover_stepfun_reading(&app2,&aid,&cfg_clone,&http,r).await);}
...
pub async fn refresh_usages_and_emit(app:&AppHandle,manual:bool)->Result<RefreshSummary,String>{
    ...
    let mut rx=providers::fetch_all_stream(&due,&http,state.refresh_slots.clone(),Some(app.clone()));
