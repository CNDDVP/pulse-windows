...
## 2026-09-17 第四轮：用户反馈项

| 反馈 | 根因 / 改动 | 验证 |
|---|---|---|
| Token 消耗点击后无报告 | `~/.gemini/tmp` 有 739MB、21,429 个聊天记录（无使用数据）被全量解析，首次扫描数分钟；全局 2 万文件上限被 gemini 占满，后续来源（Kilo/Cline）根本没被发现；切换页签卸载组件丢失结果。改为：每个来源独立发现预算；mtime 早于查询窗口起点且未入库的文件跳过解析（追加式日志的事件不会晚于 mtime）；gemini 文件前 1KB 无 `"tokens"` 视为聊天记录并记为已消费；claude `<synthetic>` 零 token 占位不计事件；TokenSpend 保持挂载 + 忙碌提示 | 真机：检查 40,444 文件（来源不再被饿死）、更新 1,246 个、按小时表格正常显示；新增 3 项测试 |
| 智谱额度文案错误 | 原按 `type` 命名（Token 额度 ×2、请求频次）。改按窗口时长命名：每5小时使用额度 / 每周使用额度；MCP 月额度被 API 报为“每分钟 1 次”的 TIME_LIMIT，改用“重置时间在 2 天以上”识别并补 30 天窗口 | 刷新缓存三条窗口与官方控制台一致；新增测试 |
