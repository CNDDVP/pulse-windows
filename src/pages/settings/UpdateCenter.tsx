...
export function UpdateCenter({hasDraft}: {hasDraft: () => boolean}) {
 ...
 if(!status)return <p className="text-xs text-[var(--text-2)]">{error||t("settings.update.loading")}</p>;
 ...
 return <div className="space-y-3 text-xs text-[var(--text-1)]">
  <div className="grid grid-cols-2 gap-2 p-3 bg-[var(--surface-2)] rounded-lg border border-[var(--border)]">
   <div>
    <span className="text-[var(--text-3)]">{t("settings.update.current_version")}</span>
    <span className="font-mono text-[var(--text-1)]">v{status.current}</span>
   </div>
   <div>
    <span className="text-[var(--text-3)]">{t("settings.update.latest_version")}</span>
    <span className="font-mono text-[var(--text-1)]">{status.offer ? `v${status.offer.version}` : isLatest ? `✓ ${t("settings.update.up_to_date")}` : t("settings.update.checking")}</span>
   </div>
   <div className="col-span-2">
    <span className="text-[var(--text-3)]">{t("settings.update.mode_label")}</span>
    <span className="text-[var(--text-1)]">{status.mode==="portable"?t("settings.update.mode_portable"):status.mode==="installed"?t("settings.update.mode_installed"):t("settings.update.mode_advanced")}</span>
   </div>
  </div>
  ...
  {status.last_result&&<p className="text-[var(--text-2)]">{t("settings.update.last_result")}{status.last_result==='awaiting_confirmation'?t("settings.update.last_result_awaiting"):status.last_result}</p>}
  {/* status.message 为 Rust 更新器 emit 的进度/结果文案，原样展示。TODO(EN-backend) */}
  <p role="status">{status.message}</p>
  {status.preferences.last_check&&<p className="text-[var(--text-3)]">{t("settings.update.last_check",{time:new Date(status.preferences.last_check*1000).toLocaleString()})}</p>}
  ...
  <p className="text-[var(--text-3)]">{t("settings.update.policy_note")}</p>
 </div>;
}
