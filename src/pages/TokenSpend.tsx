...
export function TokenSpend({ active = true, settings, onSubscriptionsSaved, onScanPathsSaved }: { active?: boolean; settings?: AppSettings | null; onSubscriptionsSaved?: (subs: Record<string, SubscriptionRecord>) => void; onScanPathsSaved?: (paths: Record<string, string[]>) => void }){
  ...
  return <section className="space-y-4"><h2 className="text-lg font-semibold">{t('spend.title')}</h2><p className="text-sm text-zinc-400">{t('spend.subtitle')}</p><div className="flex gap-1 bg-zinc-900/60 rounded-lg p-1 w-fit border border-white/5"><button className={`px-3 py-1 rounded-md text-sm cursor-pointer ${tab==='summary'?'bg-zinc-800 text-zinc-100':'text-zinc-400 hover:text-zinc-200'}`} onClick={()=>setTab('summary')}>{t('spend.tab.summary')}</button><button className={`px-3 py-1 rounded-md text-sm cursor-pointer ${tab==='trend'?'bg-zinc-800 text-zinc-100':'text-zinc-400 hover:text-zinc-200'}`} onClick={()=>setTab('trend')}>{t('spend.tab.trend')}</button><button className={`px-3 py-1 rounded-md text-sm cursor-pointer ${tab==='sessions'?'bg-zinc-800 text-zinc-100':'text-zinc-400 hover:text-zinc-200'}`} onClick={()=>setTab('sessions')}>{t('spend.tab.sessions')}</button></div><div className={tab==='summary'?'space-y-4':'hidden'}><SubscriptionPanel subs={subs} onSubsChange={setSubs} onSaved={onSubscriptionsSaved} monthCostBySource={result?.month_cost_by_source??null} monthCoverageNote={monthNote} displayCurrency={displayCurrency} fxRate={fxRate} /><ScanPathsPanel onSaved={onScanPathsSaved} /><div className="flex gap-3"><select className="bg-zinc-800 p-2 rounded disabled:opacity-40" disabled={busy} value={days} onChange={e=>handleDaysChange(Number(e.target.value))}>{[7,30,90].map(d=><option key={d} value={d}>{t('spend.days_option',{days:d})}</option>)}</select><button disabled={busy} className="bg-emerald-700 px-3 rounded disabled:opacity-40" onClick={()=>void scan()}>{busy?t('spend.scanning'):t('spend.scan')}</button><select className="bg-zinc-800 p-2 rounded" value={group} onChange={e=>handleGroupChange(e.target.value)}><option value="day">{t('spend.group.day')}</option><option value="hour">{t('spend.group.hour')}</option><option value="model">{t('spend.group.model')}</option></select><button disabled={busy||!result?.rows.length||!!exporting} className="bg-zinc-800 px-3 rounded disabled:opacity-40" onClick={()=>void exportRows('csv')}>{exporting==='csv'?t('spend.exporting'):t('spend.export_csv')}</button><button disabled={busy||!result?.rows.length||!!exporting} className="bg-zinc-800 px-3 rounded disabled:opacity-40" onClick={()=>void exportRows('json')}>{exporting==='json'?t('spend.exporting'):t('spend.export_json')}</button></div>{busy&&<p className="text-xs text-zinc-500">{t('spend.busy_hint')}</p>}{/* TODO(EN-backend)：error 为 Rust 侧消息（含「已取消」哨兵），原样展示不翻译 */}{error&&<p className="text-amber-400">{error}</p>}{exported&&<p className="text-xs text-emerald-400 break-all">{t('spend.exported_to',{path:exported})}</p>}
    {result&&<><div className="flex flex-wrap items-center gap-x-4 gap-y-1"><p className="text-zinc-400">{t('spend.scan_summary',{days:result.days,scanned:result.scanned_files,changed:result.changed_files,skipped:result.skipped_files})}{result.duration_ms!=null?t('spend.scan_duration',{ms:result.duration_ms}):''}{t('spend.sentence_end')}{result.partial?t('spend.scan_partial'):''}</p>{result.cost_estimate!=null&&<span className="text-emerald-400 text-sm font-semibold bg-emerald-950/60 px-2 py-0.5 rounded border border-emerald-800/60">{t('spend.cost_estimate_badge',{amount:formatMoney(convertFromUsd(result.cost_estimate,displayCurrency,fxRate),displayCurrency),currency:displayCurrency})}</span>}{result.cost_estimate!=null&&costNote&&<span className="text-[11px] text-zinc-500">{costNote}{t('spend.cost_original_usd')}</span>}</div>{result.coverage_gap&&<p className="text-amber-400 text-xs">{t('spend.coverage_gap')}</p>}{/* TODO(EN-backend)：notes 为 Rust 侧文案，原样展示不翻译 */}{result.notes.map((n,i)=><p key={i} className="text-xs text-zinc-500">{n}</p>)}<div className="overflow-auto"><table className="w-full text-xs text-right"><thead><tr>{(['group','input','output','cache_read','cache_write'] as const).map(k=><th key={k} className="p-2 border-b border-zinc-700">{t(`spend.col.${k}`)}</th>)}</tr></thead><tbody>{sorted.map(([label,counts])=>{
      ...
      return <Fragment key={label}>
        ...
        {open&&<tr className="bg-zinc-900/40"><td colSpan={5} className="p-2 text-left text-zinc-400">{t('spend.detail_row',{input:counts.input.toLocaleString(),output:counts.output.toLocaleString(),cache_read:counts.cache_read.toLocaleString(),cache_write:counts.cache_write.toLocaleString()})}{hit?` · ${t('spend.hit_rate',{rate:hit})}`:''}</td></tr>}
      </Fragment>;
    })}{group==='model'&&total&&<tr className="border-t border-zinc-700"><td className="p-2 text-left font-semibold text-zinc-200">{t('spend.total')}{totalRate?` · ${t('spend.total_hit_rate',{rate:totalRate})}`:''}</td>{(['input','output','cache_read','cache_write'] as const).map(k=><td key={k} className="p-2 font-semibold text-zinc-200">{total[k].toLocaleString()}</td>)}</tr>}</tbody></table></div>{group==='model'&&<p className="text-[11px] text-zinc-500">{t('spend.hit_rate_note')}</p>}{!rows.size&&<p>{t('spend.no_rows')}</p>}</>}
        ...
        {stepfunUsages.map(u => (
          <div key={u.account_id} className="p-4 bg-zinc-900/60 rounded-xl border border-white/5 space-y-2">
            <div className="flex justify-between items-center">
              <h3 className="text-sm font-semibold text-zinc-200">{t('spend.stepfun_points_title',{name:u.display_name})}</h3>
