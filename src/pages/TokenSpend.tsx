...
export function TokenSpend({ active = true }: { active?: boolean }){
  ...
  const exportRows=async(format:'csv'|'json')=>{
    ...
    setExported('');
    try{
      const path=await invoke<string>('export_ledger',{rows:result.rows,format});
