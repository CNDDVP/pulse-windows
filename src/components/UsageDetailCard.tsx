...
export function UsageDetailCard({usage,settings,placement,cardRef}:{usage:ProviderUsage;settings:AppSettings;placement?: "left" | "right" | "top" | "bottom";cardRef?:React.Ref<HTMLElement>}){
  ...
  // Round5d 项目一：深浅主题由 data-theme 驱动语义令牌（.card-panel），卡片类不再分叉。
  ...
  return <section ref={cardRef} className={`detail-scroll-container relative rounded-2xl p-4 max-h-full overflow-y-auto overflow-x-hidden w-[290px] text-xs ${animClass} card-panel text-[var(--text-1)]`}>
    {sideways && (
      <div
        className={`absolute top-6 w-3 h-3 rotate-45 pointer-events-none bg-[var(--surface-2)] ${
          isRightOfRail
            ? "-left-1.5 border-l border-b border-[var(--border-strong)]"
            : "-right-1.5 border-r border-t border-[var(--border-strong)]"
        }`}
      />
    )}
    <strong className="text-sm">{usage.display_name}</strong><p className="text-[var(--text-3)]">{usage.plan_name}</p>
    {/* Round4 项目一：实时速率行仅在账号处于工作状态时显示；None/缺省 = 渠道日志无
        usage 字段或速率不足 1，显示「—」，不编造。值为后端取整后的 tok/min。 */}
    {usage.is_active&&(
      <p className="text-[var(--text-3)] mt-0.5">{t("rail.detail.rate",{rate:typeof usage.tok_per_min==="number"&&usage.tok_per_min>=1?`${Math.round(usage.tok_per_min)} tok/min`:"—"})}</p>
    )}
    {/* TODO(EN-backend)：usage.error_message 为 Rust 侧消息，原样展示不做翻译映射。 */}
    {usage.error_message && (
      <p className="text-[var(--warn)] my-2">
        {usage.state === "stale"
    ...
    {usage.windows.map(w => {
      ...
      // Round5d 项目一：条形端点色走令牌（danger/accent），中间橙/黄档保留字面量（压力阶梯白名单）。
      const barColor = w.exhausted || w.used_percent >= red ? "var(--danger)" : w.used_percent >= amber ? "#f97316" : w.used_percent >= 50 ? "#eab308" : "var(--accent)";
      return (
        <div key={w.id} className="mt-3">
          ...
          <div className="h-1.5 bg-[var(--hover)] rounded my-1">
            <div
          ...
          <p className="text-[var(--text-3)]">{resetText(w.resets_at,now,lang)}</p>
          <p className="text-[var(--text-3)]">{timed.find(t=>t.id===w.id)?.period_note}</p>
          {settings.forecast&&usage.state==="live"&&(()=>{
            const kind=forecastKind(w,now);
            // 预警文案四档：耗尽/正常/临近/远期；黄档并入 warn 令牌（红→橙→黄阶梯）。
            const color=kind==="exhausted"?"text-[var(--danger)]":kind==="ok"?"text-[var(--ok)]":kind==="soon"?"text-[var(--warn)]":"text-[var(--warn)]";
    ...
    {usage.balances.map((b,i)=>{
      ...
      <div key={i} className="mt-3">
        ...
        {converted!=null&&note&&<p className="text-[11px] text-[var(--text-2)] mt-0.5">≈ {formatMoney(converted,displayCurrency)}{t("rail.detail.rate_note_wrap",{note})}</p>}
        {b.expires_at && (
    ...
    <p className="mt-3 text-[10px] text-[var(--text-3)]">{usage.source||t("rail.detail.source_fallback")}{usage.last_success_at&&` · ${t("rail.detail.last_success",{time:new Date(usage.last_success_at).toLocaleString()})}`}</p>
  </section>;
}
