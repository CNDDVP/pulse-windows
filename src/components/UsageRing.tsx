...
export function UsageRing({usage,settings,onHover,refreshing,onClick,onDoubleClick,lookX=0,dataKey}:
  ...
  // 戳机器人：点击时的爆裂+弹跳反馈（点击本身仍触发原有的刷新逻辑）。
  ...
  const isPureBalance = cfg?.primary_window === "__balance__";
  const effectiveUsage = isPureBalance ? { ...usage, primary_percent: null } : usage;
  const valid=["live","stale"].includes(effectiveUsage.state)&&effectiveUsage.primary_percent!==null;
  const used=effectiveUsage.primary_percent??0,pct=valid?Math.min(100,settings.display_mode==="remaining"?Math.max(0,100-used):used):0;
  ...
  const red=settings.warning_threshold,amber=red-15,custom=cfg?.ring_color??null;
  ...
  // 机器人事件（重置庆祝/完工兴奋/醒来），按槽位 key 记忆上一帧推导。
  const botEvent=useMemo(()=>useBot?detectEvent(dataKey??usage.account_id,usage,mood,cfg?.primary_window):null,
    [useBot,dataKey,usage,mood,cfg?.primary_window]);
  // 用量进警戒区（红色阈值）但还没耗尽：空闲机器人偶尔亮"！"。
  const alerting=valid&&used>=red&&!exhausted;
